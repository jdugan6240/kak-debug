...
class KakConnection:
    ...
    def cleanup(self) -> None:
        """
        Close the input fifo and cleanup the /tmp/kak-debug dir.
        """
    ...
    def _get_in_fifo_path(session: str) -> str:
        ...
        # can be undefined. Therefore, as a backup, we use the ~/.kak-debug/
        # directory.
        fifo_path = Path.home() / ".kak-debug"
        runtime_dir = xdg.xdg_runtime_dir()
        if runtime_dir is not None:
            fifo_path = Path(runtime_dir + "/kak-debug")
